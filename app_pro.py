...
st.markdown("""
<div class="guide-box">
    <b>各位同學好！我是您的導讀老師。</b><br>
    請貼上金鑰後選擇單元，我們一起來調製大杯珍奶配雞排！
</div>
""", unsafe_allow_html=True)
    ...
    if not user_key:
        ...
        with st.spinner("正在調製波霸奶茶..."):
            try:
                ...
                # 強化指令：講義例題優先
                prompt = files + [
                    f"你是有 20 年資歷的理化老師。1. 請開啟檔案一，精確找到第 {target_page} 頁。"
                    f"2. **【核心任務】**：優先詳細講解該頁面上的所有『例題』與『重點』。沒講完頁面內容不准跳出。"
                    "3. 開場白請生活化，提到『雞排配大杯珍奶』的小確幸。說各位同學好！今天老師感冒沙啞。"
                    "4. 公式如 $n=m/M$ 與化學式必須使用 LaTeX。莫耳數請用珍珠量比喻。"
                    "5. 講解完講義後，加上標籤 '[QUIZ_DATA]'。從檔案二搜尋 1 題相關會考長敘述真題，附上 (A)(B)(C)(D)、正確字母與引導提示。"
                ]
...
if st.session_state.quiz_data:
    ...
    q_main = st.session_state.quiz_data.split("正確")[0]
    st.markdown(q_main)
    ans = st.radio("選一個答案：", ["A", "B", "C", "D"], key="q_ans")
    if st.button("送出解答"):
        ...
        if ans == correct: st.success("🎯 答對了！你掌握了會考重點。")
