...
<div class="guide-box">
    ...
    3. <b>重點：務必勾選兩次同意條款</b>，然後按產生。<br>
    4. 複製金鑰，貼回下方「通行證」欄位按 Enter 即可啟動。
</div
""", unsafe_allow_html=True)
...
options = [f"第 {p} 頁：{page_titles.get(p, '單元重點內容')}" for p in r]
selected_page_str = st.selectbox("第二步：選擇精確頁碼 (不跳頁)", options)
